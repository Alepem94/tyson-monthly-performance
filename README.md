# 🍗 Dashboard Tyson Foods Mx

Dashboard interno de métricas de redes sociales, Google Ads y sentiment para **Tyson Foods Mx**.

Es la versión de una sola marca del dashboard multimarca de Grupo Megaalimentos: misma lógica y estructura, pero sin selector de marcas ni módulo de influencers.
...
La pestaña `_MARCAS` solo necesita una fila, con `marca_id = tyson`. La columna `marca` de las demás pestañas (Facebook, Instagram, etc.) debe tener el valor `tyson` (o `Tyson Foods Mx` / `Tyson Foods`, que se normalizan automáticamente) en cada fila.
...
src/
├── pages/
│   └── Dashboard.jsx       ← router principal por sección (marca fija: tyson)
├── components/
