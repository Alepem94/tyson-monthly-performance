...
export function tipoCampanaToBucket(tipoCampana) {
  ...
  if (s.includes('hard sell') || s.includes('hardsell')) return 'hard_sell'
  if (s.includes('nfl') || s.includes('super bowl') || s.includes('superbowl')) return 'nfl'
...
export function detectTipoCampanaFromName(name) {
  ...
  if (/spooky/.test(s)) return 'Spooky Nuggets'
  if (/nuggets/.test(s)) return 'Nuggets'
  if (/hard\s*sell/.test(s)) return 'Hard Sell'
  if (/\bnfl\b|\bsuper\s*bowl\b/.test(s)) return 'NFL'
